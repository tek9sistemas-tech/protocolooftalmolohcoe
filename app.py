# app.py (ATUALIZADO) — inclui gatilhos pediátricos (2–15 anos) com regra de exibição e bloqueio no cálculo
import os
import json
...
# =========================
# Referências especiais (opcional, para uso futuro no painel)
REFERENCIAS_ESPECIAIS = {
    "Miopia_Ped": {
        "ref": "CBO/SOE: Controle de Miopia Infantil",
        "desc": "Acompanhamento de crescimento axial e prevenção de alta miopia."
    }
}
...
PROTOCOLOS_MASTER = {
    ...
    # =========================
    # Protocolos Especiais e Pediátricos (2–15 anos)
    # =========================
    "Criança com Miopia (2–15 anos)": {
        "score": 4,
        "exames": [
            "Biometria ocular / Comprimento axial – AO",
            "Refração cicloplegica – AO",
            "Retinografia Simples – AO"
        ],
        "just": "Protocolo pediátrico para controle da progressão miópica e prevenção de alta miopia.",
        "biblio": "CBO/SOE: Controle de Miopia Infantil",
        "link": "https://www.cbo.com.br/"
    },
    "Criança com Miopia + Astigmatismo (2–15 anos)": {
        "score": 5,
        "exames": [
            "Topografia / Ceratoscopia – AO",
            "Biometria ocular / Comprimento axial – AO",
            "Refração cicloplegica – AO"
        ],
        "just": "Protocolo pediátrico com risco refrativo aumentado, incluindo avaliação corneana e monitoramento axial.",
        "biblio": "CBO/SOE: Controle de Miopia Infantil",
        "link": "https://www.cbo.com.br/"
    }
}
...
def protocolo(paciente_id):
    ...
    # ✅ Regra: gatilhos pediátricos aparecem apenas entre 2 e 15 anos
    gatilhos_filtrados = []
    for g in PROTOCOLOS_MASTER.keys():
        if "Criança com Miopia" in g:
            if 2 <= idade <= 15:
                gatilhos_filtrados.append(g)
        else:
            gatilhos_filtrados.append(g)
    ...
    if request.method == "POST":
        ...
        for a in achados_sel:
            # ✅ Bloqueio de segurança: mesmo se alguém tentar forçar seleção
            if "Criança com Miopia" in a and not (2 <= idade <= 15):
                continue
    ...
    return render_template(
        ...
        gatilhos=sorted(gatilhos_filtrados),
